...
const TaskItem = ({ task }) => {
  ...
  if (task.completed) {
    return (
      <div
        ...
        className="group bg-slate-50/80 p-4 md:p-5 rounded-xl border border-slate-200 opacity-60 grayscale-[0.5] transition-all cursor-pointer hover:opacity-80 overflow-hidden"
      >
        <div className="flex gap-3 md:gap-4">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center justify-between gap-2 mb-1">
              <h4 className="font-bold text-slate-800 line-through truncate">{task.title}</h4>
              <span className="text-[10px] px-2 py-0.5 rounded-full bg-slate-200 text-slate-500 font-bold uppercase tracking-wider shrink-0">Done</span>
            </div>
            {task.description && (
              <p className="text-sm text-slate-500 mb-3 line-clamp-2 italic">{task.description}</p>
            )}
  ...
  return (
    <div
      ...
      className={`group bg-white p-4 md:p-5 rounded-xl border-l-4 ${p.border} border-y border-r border-slate-200 shadow-sm hover:shadow-lg transition-all cursor-pointer overflow-hidden relative`}
    >
      <div className="flex gap-3 md:gap-4">
        <div className="mt-1 shrink-0">
          <div className="w-5 h-5 border-2 border-slate-300 rounded-md flex items-center justify-center group-hover:border-primary transition-colors"></div>
        </div>
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center justify-between flex-wrap gap-2">
            ...
            <div className={`flex items-center gap-1 transition-opacity ${showActions ? 'opacity-100' : 'opacity-100 sm:opacity-0 sm:group-hover:opacity-100'}`}>
              <button
                onClick={handlePin}
                className={`p-1 rounded-lg transition-colors ${task.pinned ? 'text-amber-500 bg-amber-500/10' : 'text-slate-400 hover:text-amber-500 hover:bg-amber-500/10'}`}
